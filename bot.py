...
# ═══════════════════════════════════════════
# QİYMƏT ÇIXARIŞI — yalnız ₼ yanındakılar
# ═══════════════════════════════════════════
def extract_prices(html: str) -> list:
    """
    YALNIZ ₼ işarəsinin birbaşa yanındakı rəqəmləri götürür.
    128GB, 2022, elan ID-ləri kimi yanlış rəqəmləri rədd edir.
    IQR metodu ilə statistik outlier-ları təmizləyir.
    """
    raw = []

    # Əsas qayda: rəqəm + maks 3 boşluq + ₼
    for p in re.findall(r'\b(\d{2,5})\s{0,3}[₼\u20bc]', html):
        val = int(p)
        if 50 < val < 30000:
            raw.append(val)

    # data-price atributu
    for p in re.findall(r'data-price=["\'](\d{2,5})["\']', html):
        val = int(p)
        if 50 < val < 30000:
            raw.append(val)
    ...
    # IQR outlier təmizləmə
    q1 = statistics.quantiles(unique, n=4)[0]
    q3 = statistics.quantiles(unique, n=4)[2]
    iqr = q3 - q1
    if iqr == 0:
        return unique
    lower = q1 - 1.5 * iqr
    upper = q3 + 1.5 * iqr
    clean = [p for p in unique if lower <= p <= upper]
    return clean if clean else unique
...
async def phone_message(model: str) -> str:
    ...
    try:
        async with httpx.AsyncClient(timeout=60, follow_redirects=True,
            ...
            log.info(f"Prices extracted: {sorted(prices)}")
