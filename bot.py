"""
═══════════════════════════════════════════════
Girov Qiymət Botu — PLAYWRIGHT VERSION (Fixed)
═══════════════════════════════════════════════
"""
...
async def scrape_tapaz_prices(model: str):
    ...
    async with async_playwright() as p:
        ...
        context = await browser.new_context(user_agent=UA, viewport={"width": 1400, "height": 900})
        ...
        try:
            ...
            prices = [p for p in prices if 80 <= p <= 15000]
...
async def phone_message(model: str):
    ...
    avg, med = statistics.mean(prices), statistics.median(prices)
    return (
        f"📱 *{model}*\n\n"
        f"📊 Ortalama : *{avg:,.0f} ₼*\n"
        f"📍 Median : *{med:,.0f} ₼*\n"
...
def kb_main():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("📱 Telefon Qiyməti", callback_data="phone")],
        [InlineKeyboardButton("🥇 Metal Qiymətləri", callback_data="metals")]
    ])
