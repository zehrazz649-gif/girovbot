...
• tap.az ikinci əl telefon qiymətləri (ScraperAPI vasitəsilə)
    ...
    for delta in range(4):
        ...
        try:
            async with httpx.AsyncClient(timeout=10) as c:
                r = await c.get(f"https://cbar.az/currencies/{d}.xml",
...
async def metal_message() -> str:
    ...
    async with httpx.AsyncClient(timeout=15, follow_redirects=True,
        ...
        # 1. gold-api.com — Railway-də işlədiyi təsdiqlənib (/debug: 200 OK)
        try:
            ...
            log.warning(f"gold-api: {e}")

        # 2. goldprice.org — ehtiyat
        if not gold:
            try:
...
async def phone_message(model: str) -> str:
    ...
    try:
        async with httpx.AsyncClient(timeout=45, follow_redirects=True,
            ...
            log.info(f"HTML SAMPLE: {r.text[500:1500]}")

            if r.status_code == 200 and len(r.text) > 200:
                # ₼ simvolu ilə qiymətlər
                for p in re.findall(r'(\d[\d\s]{1,5})\s*[₼]', r.text):
                    val = int(p.replace(" ", ""))
                    if 30 < val < 60000:
                        prices.append(val)
                # data-price atributu
                for p in re.findall(r'data-price=["\'](\d+)["\']', r.text):
                    val = int(p)
                    if 30 < val < 60000:
                        prices.append(val)
                log.info(f"Prices found: {sorted(set(prices))[:10]}")
            else:
                log.warning(f"ScraperAPI bad response: {r.text[:100]}")
    except Exception as e:
...
def kb_brands():
    ...
    for i in range(0, len(BKEYS), 2):
        ...
        rows.append(row)
    rows.append([
        InlineKeyboardButton("✏️ Özüm yazım", callback_data="manual"),
...
async def on_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if ctx.user_data.pop("awaiting_model", False):
        msg = await update.message.reply_text(
            f"🔍 *{text}* tap.az-da axtarılır...", parse_mode="Markdown")
    ...
    if any(k in text.lower() for k in kws):
        ...
        await update.message.reply_text(
            "Nə etmək istəyirsiniz?", reply_markup=kb_main())
